...
from ngs_utils.testing import BaseTestCase, info, check_call, html_wrapper, vcf_ignore_lines
...
class Test_bcbio_postproc(BaseTestCase):
    ...
    def _run_postproc(self, bcbio_dirname, eval_panel=False, parallel=False, debug=True):
        ...
        info('-' * 100)
        try:
            self._run_cmd(cmdl, bcbio_dir, results_dir, before_run_fn=lambda: shutil.copytree(bcbio_dir, results_dir, symlinks=True))
    ...
    def _check_var_in_datestamp(self, failed, datestamp_dir, caller):
        ...
        if caller != 'freebayes':  # cannot merge using `bcftools merge`: > Incorrect number of AD fields (3) at chr21:11049225, cannot merge.
            failed = self._check_file(failed, join(datestamp_dir, 'var', caller + '.vcf.gz'), vcf_ignore_lines, check_diff=False)
        return failed

    def _check_var_in_sample(self, failed, sample_dir, sample, caller):
        failed = self._check_file(failed, join(sample_dir, 'varAnnotate', sample + '-' + caller + '.anno.vcf.gz'), vcf_ignore_lines, check_diff=False)
        failed = self._check_file(failed, join(sample_dir, 'varFilter', sample + '-' + caller + '.anno.filt.vcf.gz'), vcf_ignore_lines, check_diff=False)
        failed = self._check_file(failed, join(sample_dir, 'varFilter', sample + '-' + caller + '.anno.filt.PASS.vcf.gz'), vcf_ignore_lines, check_diff=False)
    ...
    def test_03_paired_with_germline(self):
        self._test_dream_chr20('dream_chr21_paired_with_germline',
                               # callers={'vardict': ['syn3-tumor'], 'gatk-haplotype': ['syn3-normal']})
